...
import type { EsTreeNodeOfType } from "./plugin/utils/es-tree-node-of-type.js";
...
import { isNodeOfType } from "./plugin/utils/is-node-of-type.js";
import { parseSourceText } from "./plugin/utils/parse-source-file.js";
...
interface SecurityDiagnosticInput {
  ...
  readonly rule: string;
  readonly title: string;
  readonly severity: SecurityPostureDiagnostic["severity"];
  readonly message: string;
  readonly help: string;
  readonly content: string;
...
const buildDiagnostic = (input: SecurityDiagnosticInput): SecurityPostureDiagnostic => {
  const location =
...
const addDiagnostic = (
  ...
  seen: Set<string>,
  diagnostic: SecurityPostureDiagnostic,
): void => {
