    ...
    JSXAttribute(node: EsTreeNodeOfType<"JSXAttribute">) {
      ...
      if (isNodeOfType(expression, "ObjectExpression")) {
        ...
        if (match) {
          context.report({
            node: match.node,
            message: `${match.keyName} only works on one platform. On RN v7+, use the CSS \`boxShadow\` string instead (e.g. \`boxShadow: "0 2px 8px rgba(0,0,0,0.1)"\`), which works on both.`,
          });
        }
        ...
        for (const element of expression.elements ?? []) {
          ...
          if (match) {
            context.report({
              node: match.node,
              message: `${match.keyName} only works on one platform. On RN v7+, use the CSS \`boxShadow\` string instead, which works on both.`,
            });
            return;
          }
        }
