    ...
    JSXOpeningElement(node: EsTreeNodeOfType<"JSXOpeningElement">) {
      ...
      for (const attr of node.attributes ?? []) {
        ...
        for (const property of expression.properties ?? []) {
          ...
          context.report({
            node: property,
            message: `A changing ${key} on contentContainerStyle relays out the whole scroll content, so rows jump. Use \`contentInset\` instead, which the OS applies as an offset.`,
          });
          return;
        }
      }
