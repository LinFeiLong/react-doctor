...
const reportLegacyShadowProperties = (
  ...
  context.report({
    ...
    message: `Old shadow style${legacyShadowPropertyNames.length > 1 ? "s" : ""} ${quotedPropertyNames} only work on one platform. Use boxShadow for shadows that work on both on the new architecture.`,
  });
};
