  ...
  create: (context: RuleContext) => ({
    JSXElement(node: EsTreeNodeOfType<"JSXElement">) {
      ...
      for (const child of node.children ?? []) {
        ...
        context.report({
          node,
          message: `<SafeAreaView> around <${childName}> is the old way to handle safe areas. Set \`contentInsetAdjustmentBehavior="automatic"\` on the ${childName} and drop the wrapper instead.`,
        });
        return;
      }
    },
