      ...
      for (const specifier of node.specifiers ?? []) {
        ...
        if (getImportedName(specifier) === "flushSync") {
          context.report({
            node: specifier,
            message:
              "flushSync from react-dom forces an immediate update that skips View Transitions and concurrent rendering. Use startTransition for updates that are not urgent.",
          });
        }
      }
    },
