...
export interface SecurityPostureFileClassification {
  readonly bucket: "priority" | "artifact" | "other";
  readonly isGeneratedBundleByName: boolean;
}

// Pure path classification (no fs) used by the security-posture walker to
// order files priority → artifact → other and apply per-bucket file caps.
// Returns null for paths the scan never reads.
export const classifySecurityPostureFile = (
  relativePath: string,
): SecurityPostureFileClassification | null => {
...
export const shouldReadSecurityPostureContent = (
  relativePath: string,
