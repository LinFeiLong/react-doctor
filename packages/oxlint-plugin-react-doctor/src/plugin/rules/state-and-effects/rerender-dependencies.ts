      ...
      for (const element of depsNode.elements ?? []) {
        ...
        if (isNodeOfType(element, "ObjectExpression")) {
          context.report({
            node: element,
            message:
              "A new object in useEffect deps is rebuilt every render, so the effect runs again and again. Move it out or wrap it in useMemo.",
          });
        }
        if (isNodeOfType(element, "ArrayExpression")) {
          context.report({
            node: element,
            message:
              "A new array in useEffect deps is rebuilt every render, so the effect runs again and again. Move it out or wrap it in useMemo.",
          });
        }
        ...
        if (
          ...
          context.report({
            node: element,
            message:
              "Inline function in useEffect deps is rebuilt every render, so the effect runs again and again. Hoist it out of the component or wrap it in useCallback.",
          });
        }
      }
    },
