...
export const noCreateStoreInRender = defineRule<Rule>({
  ...
  create: (context: RuleContext) => ({
    CallExpression(node: EsTreeNodeOfType<"CallExpression">) {
      ...
      context.report({
        node,
        message: `\`${factory.humanLabel}(...)\` runs inside "${componentOrHookName}", so it builds a brand new store on every render. Subscribers get cut off and any saved state resets. Move the call to the top level of the file.`,
      });
    },
  }),
});
