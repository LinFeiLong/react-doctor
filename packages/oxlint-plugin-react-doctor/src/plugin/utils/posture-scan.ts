// A posture scan is a PROJECT-LEVEL security check, not an oxlint AST rule:
// @react-doctor/core's check-security-posture environment check walks the
// whole project tree, reads each candidate file once, and hands it to every
// registered posture rule's `scan`. Posture rules carry registry metadata
...
export type PostureScan = (file: ScannedFile) => PostureFinding[];
