...
import { checkSecurityPosture } from "../src/check-security-posture.js";
...
const fixtureRules = (fixtureName: string): ReadonlySet<string> =>
  rulesOf(checkSecurityPosture(path.join(FIXTURES_DIRECTORY, fixtureName)));

beforeEach(() => {
  temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "react-doctor-security-posture-"));
});
...
describe("checkSecurityPosture", () => {
  describe("Eva-grounded fixtures", () => {
    ...
    it("keeps safe postMessage, manual redirect, and static HTML patterns quiet", () => {
      expect(
        checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "ported-static-matcher-safe-patterns")),
      ).toEqual([]);
    });
    ...
    it("keeps safe agent tools and parameterized database calls quiet", () => {
      expect(
        checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "ported-agent-database-safe-patterns")),
      ).toEqual([]);
    });
    ...
    it("keeps safe path, git URL, webhook signature, and crypto patterns quiet", () => {
      expect(
        checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "ported-web-security-safe-patterns")),
      ).toEqual([]);
    });

    it("keeps redacted env examples quiet", () => {
      expect(
        checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "repository-secret-examples")),
      ).toEqual([]);
    });

    it("keeps public Supabase chat browser bundles quiet when they expose no authority fields", () => {
      expect(
        checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "real-supabase-chat-browser-bundle")),
      ).toEqual([]);
    });

    it("keeps known browser-facing analytics, license, map, and search keys quiet", () => {
      expect(checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "real-public-env-keys"))).toEqual(
        [],
      );
    });
    ...
    it("keeps server-only Supabase service-role routes quiet", () => {
      expect(
        checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "real-server-service-role-route")),
      ).toEqual([]);
    });
    ...
    it("keeps public-read private-write Supabase RLS policies quiet", () => {
      expect(
        checkSecurityPosture(
          path.join(FIXTURES_DIRECTORY, "real-supabase-public-read-private-write"),
        ),
      ).toEqual([]);
    });

    it("stays quiet on a hardened app with scoped rules and public-only browser config", () => {
      expect(checkSecurityPosture(path.join(FIXTURES_DIRECTORY, "safe-hardened-app"))).toEqual([]);
    });
  });

  it("covers the P0-P2 security posture analyzer families", () => {
    writeFile(
    ...
    const rules = rulesOf(checkSecurityPosture(temporaryRoot));
  ...
  it("adds Security metadata and source locations for direct pattern matches", () => {
    ...
    const diagnostics = checkSecurityPosture(temporaryRoot);
  ...
  it("still reports a real secret when a public key appears in the same browser bundle", () => {
    ...
    expect(rulesOf(checkSecurityPosture(temporaryRoot))).toContain("artifact-secret-leak");
  });
  ...
  it("reports source maps that expose server env names even without concrete values", () => {
    ...
    expect(rulesOf(checkSecurityPosture(temporaryRoot))).toContain("artifact-env-leak");
  });
  ...
  it("still scans high-priority SQL even when many artifact files exist first", () => {
    ...
    expect(rulesOf(checkSecurityPosture(temporaryRoot))).toContain("supabase-rls-policy-risk");
  });
  ...
  it("reports executable SVG embeds but not regular SVG image tags", () => {
    ...
    expect(rulesOf(checkSecurityPosture(temporaryRoot))).toContain("active-static-asset");
  });
